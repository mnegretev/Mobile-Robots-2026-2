...
class AStarNode(Node):
    def a_star(self, start_r, start_c, goal_r, goal_c, grid_map, cost_map, use_diagonals):
        ...
        while len(open_list) > 0 and [row, col] != [goal_r, goal_c]:
            resultado = heapq.heappop(open_list)
            [row, col] = resultado[1]
            in_closed_list[row, col] = True
            for [r, c, cost] in adjacents:
                r, c = row + r, col + c
                if r < 0 or r >= height or c < 0 or c >= width or grid_map[r,c] != 0 or in_closed_list[r,c]: continue
                g = g_values[row, col] + cost + cost_map[r, c]
                h = numpy.linalg.norm(numpy.array([r, c]) - numpy.array([goal_r, goal_c]))
                f = g + h
                if g < g_values[r, c]:
                    g_values[r, c] = g
                    f_values[r, c] = f
