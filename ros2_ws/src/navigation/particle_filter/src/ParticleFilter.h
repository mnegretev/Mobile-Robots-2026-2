...
#define FULL_NAME "JESUS ALEXIS PEREZ LEON"
...
public:
							   ...
							   float delta_x, float delta_y, float delta_t, float sigma2)
	{
		...
		for (size_t i = 0; i < particles.size(); i++)
		{
			...
			// Rotar desplazamiento al sistema de referencia de la partícula
			// y agregar ruido gaussiano
			particles[i].x = particles[i].x + delta_x * cos(theta) - delta_y * sin(theta) + rnd.gaussian(0, sigma2);
			particles[i].y = particles[i].y + delta_x * sin(theta) + delta_y * cos(theta) + rnd.gaussian(0, sigma2);
		...
		int downsampling, float sigma2)
	{
		...
		// Calcular similitud para cada partícula
		for (size_t i = 0; i < simulated_scans.size(); i++)
		{
			...
			{
				// Lectura simulada (j-ésima)
				float d_sim = simulated_scans[i].ranges[j];
				// Lectura real correspondiente (saltar 'downsampling' lecturas)
				float d_real = real_scan.ranges[j * downsampling];

				// Solo comparar si AMBAS lecturas son finitas (ignorar NaN e inf)
				if (std::isfinite(d_sim) && std::isfinite(d_real))
				{
					delta += std::abs(d_real - d_sim);
					count++;
				}
			}

			// Calcular delta promedio y similitud s = e^(-delta²/sigma²)
			if (count > 0)
				delta /= count;
	...
	static int random_choice(std::vector<double> &probabilities)
	{
		...
		// Por seguridad numérica, retornar el último índice
		...
		std::vector<geometry_msgs::msg::Pose2D> &particles, std::vector<double> &probabilities, float sigma2)
	{
		...
		for (size_t i = 0; i < particles.size(); i++)
		{
			// Elegir una partícula con probabilidad dada por probabilities
			int idx = random_choice(probabilities);

			// Copiar la partícula elegida
			resampled_particles[i] = particles[idx];

			// Agregar ruido gaussiano para evitar partículas repetidas idénticas
