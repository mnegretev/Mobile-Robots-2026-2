...
#define FULL_NAME "Palomares Quintanar Marcelin"
...
public:
	...
	for(size_t i=0; i < simulated_scans.size(); i++){
		...
		for (size_t j = 0; j < simulated_scans[i].ranges.size(); j++){
			if(std::isfinite(simulated_scans[i].ranges[j]) && std::isfinite(real_scan.ranges[j*downsampling])){
				diff += fabs(simulated_scans[i].ranges[j] - real_scan.ranges[j*downsampling]);
				sum_i++;
			}
		}
		diff = diff/sum_i;
		similarities[i] = std::exp(- (diff * diff) / (sigma2));		
	}
	...
	for(size_t i=0; i < similarities.size(); i++){
		similarities[i] = similarities[i] / sum;
	}
	...
	for(size_t i=0; i < particles.size(); i++){
		...
		resampled_particles[i] = particles[chosen];
