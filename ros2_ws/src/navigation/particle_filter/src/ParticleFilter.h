...
#define FULL_NAME "Hernandez Saldivar Hector Saul"
...
public:
    ...
    for(size_t i = 0; i < particles.size(); i++){
        ...
        float dx_global = delta_x * cos(theta) - delta_y * sin(theta);
        float dy_global = delta_x * sin(theta) + delta_y * cos(theta);

        particles[i].x += dx_global + rnd.gaussian(0, sigma2);
        particles[i].y += dy_global + rnd.gaussian(0, sigma2);
    ...
    for(size_t i = 0; i < simulated_scans.size(); i++){
        ...
        for(size_t j = 0; j < simulated_scans[i].ranges.size(); j++)
        {
            ...
            sim += exp(-(diff * diff) / (2.0f * sigma2));
            count++;
        }

        similarities[i] = (count > 0) ? (sim / count) : 0.0;
    }

    double total = 0.0;
    for(double s : similarities) total += s;

    if(total > 0.0)
        for(double& s : similarities) s /= total;
    else
        for(double& s : similarities) s = 1.0 / (double)similarities.size();
