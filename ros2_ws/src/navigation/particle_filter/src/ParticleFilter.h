...
#define FULL_NAME "DOMÍNGUEZ PALACIOS JESÚS ALEJANDRO"
...
public:
        ...
        int downsampling, float sigma2)
    {
         ...
         * IMPORTANT NOTE 1. The real sensor scans are DOWNSAMPLED. That is, only 1 out of 'downsampling'
         * scans is considered. When comparing readings, for each reading in the simulated scaxn,
         * you should skip 'downsampling' readings in the real sensor.
         * IMPORTANT NOTE 2. Both, simulated and real scans, can have infinite distances. Thus, when
         * comparing readings, ensure both simulated and real ranges are finite values. 
         */

        for (size_t i = 0; i < simulated_scans.size(); i++)
        {
            double log_similarity = 0.0;

            size_t sim_size  = simulated_scans[i].ranges.size();
            ...
            for (size_t j = 0; j < sim_size; j++)
            {
                ...
                // Only compare when both readings are finite
                if (!std::isfinite(r_sim) || !std::isfinite(r_real))
                    continue;

                float diff = r_sim - r_real;
                // Accumulate log-likelihood to avoid numerical underflow
                log_similarity += -0.5 * (diff * diff) / sigma2;
            }

            similarities[i] = exp(log_similarity);
        }

        // Normalize so that all weights sum to 1.0
        double total = 0.0;
        ...
        if (total > 0.0)
            ...
            // If all weights collapsed to 0, use a uniform distribution
            for (size_t i = 0; i < similarities.size(); i++)
    ...
    static int random_choice(std::vector<double>& probabilities)
    {
        ...
        // Walk the cumulative distribution until we exceed r
        ...
        std::vector<double>& probabilities, float sigma2)
    {
        ...
        for (size_t i = 0; i < particles.size(); i++)
        {
            ...
            resampled_particles[i].x     = particles[idx].x     + rnd.gaussian(0.0, sigma2);
            resampled_particles[i].y     = particles[idx].y     + rnd.gaussian(0.0, sigma2);
            resampled_particles[i].theta = particles[idx].theta + rnd.gaussian(0.0, sigma2);
        }
