...
public:
	...
	sub_gz_pose=this->create_subscription<geometry_msgs::msg::PoseArray>("/gazebo/justina_with_xarm/pose",1,std::bind(&ParticleFilterNode::gz_pose_callback,this,std::placeholders::_1));
	...
	while(rclcpp::ok()){
		...
		RCLCPP_INFO(this->get_logger(), "Updating localization with delta_x=%lf\tdelta_y=%lf\tdelta_a=%lf",
...
private:
	...
	this->groundtruth_x = msg->poses[3].position.x + 2.2508;
	this->groundtruth_y = msg->poses[3].position.y + 1.5;
	double w = msg->poses[3].orientation.w;
	double z = msg->poses[3].orientation.z;
	...
	delta_x = t.transform.translation.x - this->last_tf_odom.transform.translation.x;
	delta_y = t.transform.translation.y - this->last_tf_odom.transform.translation.y;
